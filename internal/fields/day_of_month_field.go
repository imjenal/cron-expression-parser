...
var _ common.CronField = &DayOfMonthField{}

func NewDayOfMonthField(value string) common.CronField {
	return &DayOfMonthField{
		Value:      value,
		StartRange: config.DayOfMonthFieldType.Min,
		EndRange:   config.DayOfMonthFieldType.Max,
	}
}

func (f *DayOfMonthField) Validate() error {
	if f.Value == "*" {
		return nil
	}
...
func (f *DayOfMonthField) Expand() ([]int, error) {
	if f.Value == "*" {
