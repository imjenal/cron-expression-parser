...
1. Clone the repository:
   ...
   ```shell
   git clone https://github.com/imjenal/cron_expr_parser.git
   cd cron_expr_parser
   ```
2. Build the application:

    ```bash 
    go build -o cron_expr_parser cmd/main.go
    ```

3. Run the application:
    ```bash 
    ./cron_expr_parser "<cron_expression>"
    ```
...
```bash 
./cron_expr_parser "*/15 3 1,5 * 1-4 /usr/bin/find"
 ```
...
- `internal/`:
    ...
    - Core logic files: `field.go`, `parser.go`, `print.go`.
